...
function checkForEmptyFields(...inputs) {
    inputs.forEach(input => {
        if (!input.value)
            showErroMessage(input, "Este campo não pode ficar vazio");
    });
}
function checkEmail(input) {
    if (!isEmail_1.default(input.value))
        showErroMessage(input, "Email inválido");
}
...
function showErroMessage(input, msg) {
    const formFileds = input.parentElement;
    const errorMessage = formFileds.querySelector(".error-message");
